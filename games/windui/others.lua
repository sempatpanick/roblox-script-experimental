...
do
    local ok, result = pcall(function()
        return require("../../windui_library")
    end)

    if ok then
        WindUILibrary = result
    else
        if cloneref(RunService):IsStudio() then
            WindUILibrary = require(cloneref(ReplicatedStorage):WaitForChild("windui_library"))
        else
            local okGet, source = pcall(function()
                return game:HttpGet(baseURL .. "/windui_library.lua")
            end)
            assert(okGet and type(source) == "string", "[windui/others] failed to load windui_library")
            source = stripSourceBom(source)
            WindUILibrary = (loadstring or load)(source, "windui_library")()
        end
    end
end
...
do
    ...
    if ok and type(loader) == "function" then
        ...
        assert(okGet and type(source) == "string", "[windui/others] failed to load functions/load_module")
        source = stripSourceBom(source)
        loadFunctionModule = (loadstring or load)(source, "functions/load_module")()
    end
end

local notifyMod = loadFunctionModule("windui/notify")
local formatMod = loadFunctionModule("instance/format")
local compatMod = loadFunctionModule("windui/compat")
local mountNotify = notifyMod.create(WindUILibrary)

end

-- */  Local Player Tab (module)  /* --
local LOCAL_PLAYER_TAB_REPO = baseURL .. "/tabs/windui/local_player_tab.lua"
local function loadCreateLocalPlayerTab(repoUrl)
    local okReq, mod = pcall(function()
        return require("../../tabs/windui/local_player_tab")
    end)
...
-- */  Objects Tab (module)  /* --
local OBJECTS_TAB_REPO = baseURL .. "/tabs/windui/objects_tab.lua"
local function loadCreateObjectsTab(repoUrl)
    local okReq, mod = pcall(function()
        return require("../../tabs/windui/objects_tab")
    end)
    if okReq and type(mod) == "function" then
        return mod
    end
...
-- */  Teleport Tab (module)  /* --
local TELEPORT_TAB_REPO = baseURL .. "/tabs/windui/teleport_tab.lua"
local function loadCreateTeleportTab(repoUrl)
    local okReq, mod = pcall(function()
        return require("../../tabs/windui/teleport_tab")
    end)
    if okReq and type(mod) == "function" then
        return mod
    end
...
local RECORDING_TAB_REPO = baseURL .. "/tabs/windui/recording_tab.lua"
local function loadCreateRecordingTab(repoUrl)
    local okReq, mod = pcall(function()
        return require("../../tabs/windui/recording_tab")
    end)
    if okReq and type(mod) == "function" then
        return mod
    end
...
-- */  Config Tab (module)  /* --
local CONFIG_TAB_REPO = baseURL .. "/tabs/windui/config_tab.lua"
local function loadCreateConfigTab(repoUrl)
    local okReq, mod = pcall(function()
        return require("../../tabs/windui/config_tab")
    end)
    if okReq and type(mod) == "function" then
        return mod
    end
...
-- */  Window  /* --
local Window = WindUILibrary:CreateWindow({
...
local UiWindow = compatMod.wrapWindow(Window)
...
-- */  Local Player Tab  /* --
do
    local okLocalPlayer, localPlayerErr = pcall(createLocalPlayerTab, Window, mountNotify)
    if not okLocalPlayer then
        warn("[windui/others] Local Player tab failed:", localPlayerErr)
        mountNotify({
            Title = "Local Player",
            Content = "Tab failed to load: " .. tostring(localPlayerErr),
            Icon = "x",
        })
    end
end
...
createConfigTab(Window, mountNotify, {
    ...
    windUILibrary = WindUILibrary,
})

-- */  Avatar Tab  /* --
do
    local AvatarTab = UiWindow:CreateTab("Avatar", 7076763398)
