--[[
  Recording tab module for WindUI scripts.
  Reuses tabs/rayfield/recording_tab.lua logic via windui/compat.

  Usage:
    createRecordingTab(Window, mountNotify, recordingsDir)
]]
...
local createRayfieldRecordingTab = loadRayfieldTab("recording_tab")

local function createRecordingTab(windowRef, notifyFn, recordingsDir)
	return createRayfieldRecordingTab(compatMod.wrapWindow(windowRef), notifyFn, recordingsDir)
end
